...
- **jobs**: Defines the jobs to be run as part of the workflow.
    - **checks**: Identifies a job within the workflow, named "checks".
        ...
        - **steps**: Lists the steps to be executed as part of the job.
            ...
            - **- run: uv sync --group=check**: Executes the command to install dependencies specified under the "checks" group with uv.
            - **- run: uv run just check-code**: Executes a task to check code quality.
            - **- run: uv run just check-type**: Executes a task to check type annotations.
            - **- run: uv run just check-format**: Executes a task to check code formatting.
            - **- run: uv run just check-security**: Executes a task to check security vulnerabilities.
            - **- run: uv run just check-coverage**: Execute a task to check test coverage.
